...
def generate_roku_feed():
    """Generate a JSON feed for Roku Direct Publisher"""
    ...
    feed["categories"] = [
        ...
        for name in sorted(category_names)  # Sort for consistency
    ]

    return feed

def publish_to_roku():
    """Generate and save Roku JSON feed locally"""
    feed = generate_roku_feed()
    feed_path = os.path.join(settings.BASE_DIR, "roku-feed.json")

    try:
        with open(feed_path, 'w', encoding='utf-8') as f:
            json.dump(feed, f, indent=2)
        print(f"✅ Roku feed successfully saved to {feed_path}")
    except IOError as e:
        print(f"❌ Error saving Roku feed: {e}")

    return feed_path

class RokuFeedAPI(View):
    """API Endpoint: Returns the Roku JSON feed from a saved file"""
